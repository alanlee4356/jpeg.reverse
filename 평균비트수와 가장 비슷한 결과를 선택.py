...
def quantize(block, inverse=False):  # jpeg퀄리티는 QUANTIZATION_TABLE값에 반비례
    ...
    if inverse:
        quantarr = block * (quantization_table/12)
        # return quantarr
        for i in range(0, 8):
            for j in range(0, 8):
        ...
        while current_slice:
            if (current_slice in
                ...
                if key == ZRL:
                    ...
                    keys.append(key)
                    return keys, bit_seq[current_idx+4:], error
                else:
            ...
            # )
            print("cannot find prefix in huffman table")
            error = True
            return keys, bit_seq[current_idx+4:],error
    return keys
...
def decoding1(a10):#flip
    overflowerr = False
    breakerr = False
    l = 0 #decoding 할때 사용한 bits수
    a11, remain_bits = decode_dc_huffman(a10)  # a11이 dc value
    a12, remain_bits, breakerr = decode_ac_huffman(remain_bits)
    if breakerr == True:
        return -99, remain_bits, -99
    l += len(a10) - len(remain_bits)
    a13,overflowerr = decode_run_length(a12)
    if overflowerr == True:#decode runlength에서 인덱스 초과하면 -1보내고 decoding밖으로 내보내서 오류 출력
        return -1, remain_bits, -1
    a13[0] = a11
    a14 = izigzag(a13)
    a15 = quantize(a14, True)
    a16 = idct2d(a15)
...
#         a17,encoded_bits,l = decoding1(encoded_bits)
...
for i in range(0, 512, 8):#플립된 비트 디코딩 평균비트수
    for j in range(0, 512, 8):
        ...
        print(i,j)
        while n<6:
            fliped_bits = flip(encoded_bits[:n*length])
            encoded_bits1 = replace(encoded_bits,fliped_bits, n*length)
            a17,encoded_bits1,l = decoding1(encoded_bits1)#replace 하는거 다시확인
            print(pixel_diff(a17))#삭제해야함
            if pixel_diff(a17)>diffmax:#삭제해야함
                diffmax = pixel_diff(a17)#삭제해야함
            print(diffmax)#삭제해야함

            if l == -1:# runlength 범위초과로 디코딩안될때 
                l = -1
            elif l == -99:
                error = True
                break
            elif mini > abs(l-length):
                mini = abs(l-length)
                minin = n#평균값에 근접한 n값 저장
        ...
        if minin == 0:
            ...
            print("1")
            fliped_bits = flip(encoded_bits[:minin*length])
            encoded_bits1 = replace(encoded_bits,fliped_bits,minin*length)
            a17, encoded_bits,l = decoding1(encoded_bits1)
            print(a17)
            arr1[i:i+8, j:j+8] = a17
