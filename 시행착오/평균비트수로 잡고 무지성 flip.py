...
def decoding1(a10):#flip
    ...
    a13,error = decode_run_length(a12)
    if error == True:#decode runlength에서 인덱스 초과하면 -1보내고 decoding밖으로 내보내서 오류 출력
...
for i in range(0, 512, 8):#플립된 비트 디코딩 평균비트수
    for j in range(0, 512, 8):
        ...
        while True:
            ...
            if n>5 and error ==False: # n이 5까지 갔는데 정답이 안나오면 0플립으로 
                n = 0
                error = True
                continue

            elif n>5 and error == True:
                break
            ...
            elif n*length > l or (n == 0 and length > l) or error==True: #평균블록비트수로 플립했는데 그안에 eob를 찾았을경우에는 while 탈출
                encoded_bits = encoded_bits1
                if n == 0:
                    isflip = False
                    print("random bit",0)
                else :
