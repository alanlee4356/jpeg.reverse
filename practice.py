...
# # #     i = 0
# # #     output = np.zeros((v_max * h_max),np.uint)
...
# # arr = np.zeros((8,8),np.uint)
# # def zigzag(arr):
# #     flat_arr = arr.flatten()
# #     z = 0
# #     zig_arr = np.zeros(64,np.uint8)
...
# def quantize(block, inverse=False):
...
# def int_to_binary(decarr):# decarr에있는 13을 1101로 배열에 입력시켜야함
#     binarr = np.zeros(np.uint8)
#     for i in range(0,64):
#         k= bin(decarr[i])
#         binarr.append = k[2:]   
#         print(binarr[i])
...
# plt.imshow(newimg)#그레이스케일은 cmap=plt.cm.gray설정필요
...
# for i in range(0, 512, 8):#디코딩확인용
#     for j in range(0, 512, 8):
#         a1 = arr[i:i+8, j:j+8]
#         tmp = encodingtest(a1)

#         a17,encoded_bits,a13 = decodingtest(encoded_bits)
        
#         arr1[i:i+8, j:j+8] = a17
#         # 501번째 줄에 EOB만나면 ac디코딩 끝내는거로 변경해야함 현재 문제는 모든 비트를 다 합쳐놨을때 ac가 eob만나고 다음 dc디코딩 시켜야함
...
# a = '{:0{padding}b}'.format(-10, padding=4)
# print(bin(-10))
# print(a)
...
def sorting(encoded_bits):
    index = len(encoded_bits)-4
    end = len(encoded_bits)
    while True:
        if encoded_bits[index:index+4] =='1010':
            print(encoded_bits[index+4:end])#여기서부터 decoding진행하면됨. 만약에 디코딩이완전하지않으면 하나더 이전 1010으로 진행
            while index>0:
                index-=1
                if encoded_bits[index:index+4] == '1010':
                    try :
            ...
            print(flip(encoded_bits[index+4:end]))#여기서부터 decoding진행하면됨. 만약에디코딩이 완전하지않으면 하나더 이전 1010으로 진행
            while index>0:
                index-=1
                if encoded_bits[index:index+4] == '1010':
                    try :
                        tmp,remain_tmp=decode_dc_huffman(flip(encoded_bits[index+4:end]))
                        decode_ac_huffman(remain_tmp)
                        print('이 1010은 이전블록의 eob구만')
                        encoded_bits_tmp = replace(encoded_bits,flip(encoded_bits[index+4:end]),index+4,end-index-4)
...
encoded_bits = '10111111010000010011010010000100110100100101001110010111101011010'
encoded_bits1 = '10111111010000010011010010000100110101011010101110010111101011010'
# if encoded_bits[-4:] == '1010':
#     print('yes')
sorting(encoded_bits1)

# while index>0:#역방향으로 1010을 찾는 함수
#     index -= 1
#     if encoded_bits[index:index+4] =='1010':
#         print(encoded_bits[index+4:end])#여기서부터 decoding진행하면됨. 만약에 디코딩이 완전하지않으면 하나더 이전 1010으로 진행
#         try :
#             tmp,remain_tmp=decode_dc_huffman(encoded_bits[index+4:end])
#             decode_ac_huffman(remain_tmp)
#             print('이 1010은 이전블록의 eob구만')
...
#     elif encoded_bits[index:index+4] =='0101':#지금 여기 오류있음
#         print(flip(encoded_bits[index+4:end]))#여기서부터 decoding진행하면됨. 만약에 디코딩이 완전하지않으면 하나더 이전 1010으로 진행
#         try :
#             tmp,remain_tmp=decode_dc_huffman(flip(encoded_bits[index+4:end]))
#             decode_ac_huffman(remain_tmp)
#             print('이 1010은 이전블록의 eob구만')
...
#             print('이 1010은 이전블록의 eob가 아니구만')
        ...
        #decoding 해보기 1010까지 남는비트없이 디코딩되면 굳
        #만약 그렇지않으면 다음 1010을 찾아서 떠나기
...
# for i in range(0, 512, 8):  # 역방향 디코딩
#     for j in range(0, 512, 8):
#         print(i,j)
